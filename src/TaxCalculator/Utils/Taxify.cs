...
namespace TaxCalculator.Utils
{
    public static class Taxify
    {
        ...
        {
            List<string> TransactionLog = new List<string>();  // List to hold the log entries
            ...
            {
                double amount = transaction.TransactionType?.ToUpper() == "SELL"
                ...
                else if (transaction.TransactionType.ToUpper() == "SELL")
                {
                    if (!tokenBalances.ContainsKey(transaction.AmountAssetType) || tokenBalances[transaction.AmountAssetType] - amount < -TOLERANCE)
                    {
                        ...
                        foreach (var t in transactions)
                        {
                            ...
                            {
                                double availableBalanceBefore = tokenBalances.ContainsKey(t.AmountAssetType) ? tokenBalances[t.AmountAssetType] : 0;

                                // Adjust the balance after this transaction
                                double transactionAmount = t.TransactionType?.ToUpper() == "SELL"
                                    ? -Math.Abs(t.Amount ?? 0)
                                    : t.Amount ?? 0;

                                tokenBalances[t.AmountAssetType] = availableBalanceBefore + transactionAmount;
                                double availableBalanceAfter = tokenBalances[t.AmountAssetType];

                                TransactionLog.Add($"SEQ {t.Sequence}: {t.TransactionType.ToLower()} {t.Amount} {t.AmountAssetType}. Available after: {availableBalanceAfter} {t.AmountAssetType}");
                            }
                        ...
                        // Flag the invalid transaction
                        transaction.FlaggedForReview = true;
                        var amountAvailable = tokenBalances.ContainsKey(transaction.AmountAssetType) ? tokenBalances[transaction.AmountAssetType] : 0;
                        var amountAttemptedToSell = amount;
                        var difference = amountAvailable - amount;

                        StringBuilder sb = new StringBuilder();
                        sb.AppendLine($"Insufficient funds available to sell {amount} {transaction.AmountAssetType}. Available: {amountAvailable} {transaction.AmountAssetType}");
                        sb.AppendLine($"Attempted to sell: {amountAttemptedToSell} {transaction.AmountAssetType}.");
                        sb.AppendLine($"Difference: {difference} {transaction.AmountAssetType}");
                        sb.AppendLine($"<hr>");
                        TransactionLog.ForEach(x => sb.AppendLine($"{x}<br>"));
                        transaction.InternalNotes = sb.ToString();
